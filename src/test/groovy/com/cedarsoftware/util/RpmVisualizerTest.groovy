...
class RpmVisualizerTest
{
    ...
    void testBuildGraph_checkVisInfo()
    {
        Map scope = [_effectiveVersion: ApplicationID.DEFAULT_VERSION,
                     product          : 'WorkersCompensationProduct',
                     policyControlDate: '2017-01-01',
                     quoteDate        : '2017-01-01',
                     coverage         : 'FELACoverage',
                     risk             : 'WCOps']
        ...
        assert ['GeneralLiabilityOps', 'ProductLocation', 'StateOps', 'WCOps'] as Set == visInfo.availableScopeValues['sourceRisk']
    ...
    void testBuildGraph_canLoadTargetAsRpmClass()
    {
        Map scope = [_effectiveVersion: ApplicationID.DEFAULT_VERSION,
                     ...
                     policyControlDate:'2017-01-01',
                     quoteDate:'2017-01-01',
                     coverage: 'ClassCodeCoverage',
                     sourceCoverage: 'FELACoverage',
                     risk: 'WCOps']
        String startCubeName = 'rpm.class.Coverage'
    ...
    void testBuildGraph_checkStructure()
    {
        ...
        assert nodes.find { Map node -> 'FELACoverage' == node.label}
        assert nodes.find { Map node -> 'IncreasedLimitCoverage' == node.label}
        assert nodes.find { Map node -> 'ClassCodeCoverage' == node.label}
        assert nodes.find { Map node -> "${UNABLE_TO_LOAD}Location".toString() == node.label}
        assert nodes.find { Map node -> 'Valid values for field Coverages on FELACoverage' == node.title}

        assert edges.find { Map edge -> 'FELACoverage' == edge.fromName && 'rpm.enum.Coverage.Coverages' == edge.toName}
        assert edges.find { Map edge -> 'rpm.enum.Coverage.Coverages' == edge.fromName && 'IncreasedLimitCoverage' == edge.toName}
        assert edges.find { Map edge -> 'rpm.enum.Coverage.Coverages' == edge.fromName && 'ClassCodeCoverage' == edge.toName}
        assert edges.find { Map edge -> 'ClassCodeCoverage' == edge.fromName && "${UNABLE_TO_LOAD}Location".toString() == edge.toName}
    }
    ...
    void testBuildGraph_checkNodeAndEdge()
    {
        ...
        Map classCodeCoverageScope = new CaseInsensitiveMap(startScope)
        classCodeCoverageScope.coverage = 'ClassCodeCoverage'
        classCodeCoverageScope.sourceCoverage = 'FELACoverage'

        Map availableClassCodeCoverageScope = new CaseInsensitiveMap(classCodeCoverageScope)
        availableClassCodeCoverageScope.sourceFieldName = 'Coverages'
        ...
        //Edge from top level node to enum
        Map edge = edges.find { Map edge -> 'FELACoverage' == edge.fromName &&
        ...
        //Enum node under top level node
        node = nodes.find { Map node1 -> 'Valid values for field Coverages on FELACoverage' == node1.title}
        assert 'rpm.enum.Coverage.Coverages' == node.cubeName
        assert null == node.label
        assert 'Coverages' == node.fromFieldName
        assert 'Valid values for field Coverages on FELACoverage' == node.detailsTitle1
        assert null == node.detailsTitle2
        assert 'COVERAGE_ENUM' == node.group
        assert '2' == node.level
        assert 'rpm.class.Coverage' == node.sourceCubeName
        assert 'FELACoverage' == node.sourceDescription
        ...
        assert nodeDetails.contains(DETAILS_LABEL_UTILIZED_SCOPE_WITHOUT_ALL_TRAITS)
        assert nodeDetails.contains(DETAILS_LABEL_AVAILABLE_SCOPE)
        assert nodeDetails.contains("${DETAILS_LABEL_FIELDS}</b><pre><ul><li>ClassCodeCoverage</li><li>IncreasedLimitCoverage</li></ul></pre>")
        assert !nodeDetails.contains(DETAILS_LABEL_FIELDS_AND_TRAITS)
        assert !nodeDetails.contains(DETAILS_LABEL_REASON)
        assert !nodeDetails.contains(DETAILS_LABEL_NOTE)
        assert !nodeDetails.contains(DETAILS_LABEL_CLASS_TRAITS)
        ...
        edge = edges.find { Map edge1 -> 'rpm.enum.Coverage.Coverages' == edge1.fromName &&
                'ClassCodeCoverage' == edge1.toName}
        assert 'ClassCodeCoverage' == edge.fromFieldName
        assert '3' == edge.level
        assert !edge.label
        assert "Valid value ClassCodeCoverage cardinality ${V_MIN_CARDINALITY}:${V_MAX_CARDINALITY}".toString() == edge.title

        //Target node of top level node
        node = nodes.find { Map node1 -> 'ClassCodeCoverage' == node1.label}
        assert 'rpm.class.Coverage' == node.cubeName
        assert 'ClassCodeCoverage' == node.fromFieldName
        assert 'Coverage' == node.title
        assert 'Coverage' == node.detailsTitle1
        assert 'ClassCodeCoverage' == node.detailsTitle2
        assert 'COVERAGE' == node.group
        assert '3' == node.level
        assert 'rpm.enum.Coverage.Coverages' == node.sourceCubeName
        assert 'field Coverages on FELACoverage' == node.sourceDescription
        assert ['Coverage', 'Deductible', 'Limit', 'Premium', 'Rate', 'Ratefactor', 'Role'] == node.typesToAdd
        assert true == node.showCellValuesLink
        assert false == node.showCellValues
        assert true == node.cellValuesLoaded
        assert classCodeCoverageScope == node.scope
        assert availableClassCodeCoverageScope == node.availableScope
        nodeDetails = node.details as String
        assert nodeDetails.contains(DETAILS_LABEL_UTILIZED_SCOPE_WITHOUT_ALL_TRAITS)
        assert nodeDetails.contains(DETAILS_LABEL_AVAILABLE_SCOPE)
    ...
    void testGetCellValues_classNode_showCellValues()
    {
        ...
        Map oldNode = [
                ...
                fromFieldName: 'ClassCodeCoverage',
                title: 'rpm.class.Coverage',
                level: '3',
                label: 'ClassCodeCoverage',
                scope: nodeScope,
                showCellValues: true,
        ...
        Map node = nodes.find { Map node -> 'ClassCodeCoverage' == node.label}
        assert true == node.showCellValuesLink
        ...
        assert !nodeDetails.contains(DETAILS_LABEL_REASON)
        assert !nodeDetails.contains(DETAILS_LABEL_NOTE)
        assert nodeDetails.contains("${DETAILS_LABEL_CLASS_TRAITS}</b><pre><ul><li>r:exists: true</li><li>r:name: ClassCodeCoverage</li><li>r:scopedName: ClassCodeCoverage</li></ul></pre><br><b>")
    }
    ...
    void testGetCellValues_enumNode_showCellValues()
    {
        Map scope = [_effectiveVersion: ApplicationID.DEFAULT_VERSION,
          ...
          Map oldNode = [
                ...
                fromFieldName: 'FELACoverage',
                title: 'Valid values for field Coverages on FELACoverage',
                level: '2',
        ...
        Map node = nodes.find { Map node1 -> 'Valid values for field Coverages on FELACoverage' == node1.title}
        assert true == node.showCellValuesLink
        assert true == node.showCellValues
        assert true == node.cellValuesLoaded
        String nodeDetails = node.details as String
        ...
        assert nodeDetails.contains(DETAILS_LABEL_UTILIZED_SCOPE)
        assert nodeDetails.contains(DETAILS_LABEL_AVAILABLE_SCOPE)
        assert nodeDetails.contains("${DETAILS_LABEL_FIELDS_AND_TRAITS}</b><pre><ul><li><b>ClassCodeCoverage</b></li><pre><ul><li>r:declared: true</li><li>r:exists: true</li><li>r:name: ClassCodeCoverage</li><li>v:max: 999999</li><li>v:min: 0</li></ul></pre><li><b>IncreasedLimitCoverage</b></li><pre><ul><li>r:declared: true</li><li>r:exists: true</li><li>r:name: IncreasedLimitCoverage</li><li>v:max: 1</li><li>v:min: 0</li></ul>")
        assert !nodeDetails.contains(DETAILS_LABEL_REASON)
        assert !nodeDetails.contains(DETAILS_LABEL_NOTE)
    ...
    void testBuildGraph_initialPromptForScope()
    {
        ...
        assert message.contains('GeneralLiability')
        assert message.contains('UmbrellaProduct')
        assert message.contains('WorkersCompensationProduct')
    }
    ...
    private static void checkAdditionalScopeIsRequiredMessage(String message)
    {
        ...
        assert message.contains('GeneralLiabilityOps')
        assert message.contains('ProductLocation')
        assert message.contains('StateOps')
        assert message.contains('WCOps')
    }
    ...
    void testBuildGraph_missingRequiredScopeWithNoPreLoadedAvailableScopeValues()
    {
        try
        {
            ...
            assert 1 == messages.size()
            String message = messages.first()
            assert message.contains("${ADDITIONAL_SCOPE_REQUIRED_TO_LOAD}WorkersCompAssessmentCoverage of type Coverage.")
            assert message.contains("${ADDITIONAL_SCOPE_REQUIRED_TO_LOAD}WorkCompStateCoverage of type Coverage.")
            assert message.contains("${ADDITIONAL_SCOPE_REQUIRED_TO_LOAD}ClassCodeCoverage of type Coverage.")
            assert message.contains("${ADDITIONAL_SCOPE_REQUIRED_TO_LOAD}FELACoverage of type Coverage.")
            assert message.contains("${ADDITIONAL_SCOPE_REQUIRED_TO_LOAD}AdmiraltyCoverage of type Coverage.")
            checkMissingRequiredScopeMessage(message)

            List<Map<String, Object>> nodes = (graphInfo.visInfo as RpmVisualizerInfo).nodes as List
            Map node = nodes.find {Map node ->  "${MISSING_SCOPE}FELACoverage".toString() == node.label}
            assert 'Coverage' == node.title
            assert 'Coverage' == node.detailsTitle1
            assert 'FELACoverage' == node.detailsTitle2
            String nodeDetails = node.details as String
            assert nodeDetails.contains("*** ${UNABLE_TO_LOAD}fields and traits for FELACoverage")
            assert nodeDetails.contains(DETAILS_LABEL_REASON)
            assert message.contains("${ADDITIONAL_SCOPE_REQUIRED_TO_LOAD}FELACoverage of type Coverage.")
    ...
    void testBuildGraph_missingDeclaredRequiredScope()
    {
        ...
        try
        {
            ...
            Map node = nodes.find {Map node ->  "${MISSING_SCOPE}WorkersCompAssessmentCoverage".toString() == node.label}
            assert 'Coverage' == node.title
            assert 'Coverage' == node.detailsTitle1
            assert 'WorkersCompAssessmentCoverage' == node.detailsTitle2
            String nodeDetails = node.details as String
            assert nodeDetails.contains("*** ${UNABLE_TO_LOAD}fields and traits for WorkersCompAssessmentCoverage")
            assert nodeDetails.contains("${DETAILS_LABEL_REASON}")
            assert nodeDetails.contains("${ADDITIONAL_SCOPE_REQUIRED_TO_LOAD}WorkersCompAssessmentCoverage of type Coverage.")
